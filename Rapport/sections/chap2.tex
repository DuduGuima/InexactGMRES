...
Our $x_{k}$, the $k-th$ approximation to $x$, if chosen as the answer of the minisation problem of $r_{k}$. Since we make the projections in $\mathcal{K}_{k}(A,b)$, $x_{k} = K_{k}y_{k}$, our answer is given by the problem:
...
But as said in the previus chapters,${b,Ab, \dots, A^{k}b}$ has approximately colinear vectors, and we choose the base from the Arnoldi's Method instead.
...
Thus, $y_{k}$ which appears in \ref{eq:init_gmres}, is found as the solution of the residual's minimisation problem  in \ref{eq:final_gmres}:

\begin{equation}\label{eq:y_gmres}
    y_{k} = \min_{y \in \mathbb{C}^{k}} \norm{\beta e_{1} - H_{k}y}
\end{equation}
