...
range_values= [10.0^(-i) for i in 2:6]
...
for k=1:length(range_values)
    ...
    # benchr_exact = @benchmark InexactGMRES.test_gmres($H,$b,tol=range_values[$k]) 
    # benchr_approx = @benchmark igmres($H,$b,tol=range_values[$k]) 

    # results_approx[k] = mean(benchr_approx).time
    # results_exact[k] = mean(benchr_exact).time

    #y_exact= gmres(H,b;reltol=1e-5)
    y_exact,res_exact,n_it_exact = InexactGMRES.test_gmres(H,b,tol=range_values[k])
    y_approx,res_approx,n_it_approx = igmres(H,b,tol=range_values[k])
...
# p1 = plot(range_values,[results_approx results_exact].*1e-9,title="Total execution time",label=["Inexact GMRES" "Exact GMRES"],legend=true,xaxis=:log,xflip=true)
# xlabel!(p1,"Product Tolerance")
# ylabel!(p1,"Ex. time[ns]")


# p2 = plot(range_values,speed_up, title="Speed up", legend=false,xaxis=:log,xflip=true)
# xlabel!(p2,"Product Tolerance")
# ylabel!(p2,"Speed up")
...
p1 = scatter(range_values, rel_error_sol, title="Relative error between exact and inexact solution",legend=false,yaxis=:log,xaxis=:log,xflip=true)
xlabel!(p1,"Tolerance")
ylabel!(p1,"Relative Error")
ylims!(p1,10.0^(-7),10.0^(-2))
p2 = plot(range_values,[n_iterations_approx n_iterations_exact],label= ["Inexact GMRES" "Exact GMRES"], title="Number of Iterations",legend=true,xaxis=:log,xflip=true)
xlabel!(p2,"Tolerance")
